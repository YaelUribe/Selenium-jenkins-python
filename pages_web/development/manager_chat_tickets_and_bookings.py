...
class ChatTicketsManager(BasePage):
    ...
    status_filter = (By.XPATH, "//app-conversations-list//form/div[1]//div[@class='ng-value-container']")
    ...
    select_technician_option = (By.XPATH, "//ng-dropdown-panel//span[contains(text(), 'Technician Dvora')]")
    ...
    disconnect_utilities_tag = (By.XPATH, "//div[@class= 'manager-overlay-container active']//h2[contains(text(), 'Disconnect Utilities')]")
    ...
    next_week_arrow = (By.XPATH, "//full-calendar/div/div[3]")
    start_time = (By.XPATH, "//tr[21]/td[2]")
    end_time = (By.XPATH, "//tr[23]/td[2]")
    ...
    property_selector_service = (By.XPATH, "//ng-select[@formcontrolname= 'property']//input")
    unit_service = (By.XPATH, "//ng-select[@formcontrolname= 'unit']//input")
    ...
    def create_moveout_ticket(self, date):
        tenant_name = self.get_text(self.member_name)
        clicks = [self.more_options_btn, self.create_ticket_option, self.ticket_type,
        ...
        for y in clicks_2:
            ...
            if y == self.select_date_input:
                try:
                    ...
                    continue
                self.send_keys(y, self.date_calendar_2(date))
                time.sleep(0.8)
            elif y == self.member_select:
                self.send_keys(y, tenant_name)
                time.sleep(0.8)
    ...
    def reserve_a_space(self):
        ...
        try:
            ...
            self.scroll_to_location_web(self.reservation_calendar)
            self.do_click(self.next_week_arrow)
            time.sleep(0.5)
    ...
    def request_service(self):
        ...
        for y in clicks_2:
            self.scroll_to_location_web(y)
            if y == self.property_selector_service:
                self.send_keys(y, "DVORA 175")
                time.sleep(0.5)
                self.press_enter_web()
                time.sleep(1.2)
            elif y == self.preferred_service_days:
                self.click_single_random_option_web(y)
                time.sleep(0.5)
            elif y == self.unit_service:
                try:
                    self.send_keys(y, "0704")
                    time.sleep(0.5)
                    self.press_enter_web()
                    time.sleep(1.2)
                except TimeoutException:
