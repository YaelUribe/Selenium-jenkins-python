...
class ManagerLoginPage(BasePage):
    ...
    user_icon = (By.XPATH, "//div[@class= 'user-profile']")
    ...
    def login(self):
        """login method"""
        self.send_keys(self.email, self.manager_email)
        self.do_click(self.next_btn)
        time.sleep(0.8)
        self.send_keys(self.password, self.manager_pass)
        self.do_click(self.next_btn)
        time.sleep(12)
        return self.is_visible(util.manager_home_icon)
